...
async def generate_digest(
    ...
    prompt = (
        ...
        "Produce a daily briefing. STRICT SYNCHRONIZATION RULE: "
        "Every claim or evidence cited in the Core Insight (核心洞察) MUST correspond to "
        "a specific bullet item in Today's News (今日要闻) with an x.com link. "
        "Readers must be able to verify every analytical claim by clicking a link in Today's News. "
        "Generate in this order: FIRST write Today's News (select the most newsworthy items), "
        "THEN write Core Insight that ONLY references those exact news items.\n\n"
        ...
        "📰 今日要闻 | " + date + "\n\n"
        "🌱 ARKREEN\n"
        "• [具体新闻条目，附 x.com 链接。每项格式：新闻摘要 → x.com/i/web/status/TWEET_ID]\n\n"
        "💚 绿色比特币\n"
        "• [具体新闻条目，附 x.com 链接]\n\n"
        "👜 TLAY\n"
        "• [具体新闻条目，附 x.com 链接]\n\n"
        ...
        "Rules: No bullet points in body text. No inline links. Pure analysis. Min 200 Chinese characters.\n"
        "IMPORTANT: In Chinese text, always write '绿色比特币' instead of 'GreenBTC'. Keep ARKREEN, TLAY, AI Renaissance as-is.\n"
        "MANDATORY: End the insight with a '消息来源' block listing EVERY 今日要闻 bullet item "
        "that supports your analysis, using the exact same links from 今日要闻. Format:\n"
        "消息来源\n"
        "• [来源描述] → [x.com link]\n"
        "• ...\n]\n"
        "===INSIGHT_ZH_END===\n\n"
        ...
        "[Same analytical content in English. 2-3 paragraphs. No inline links. Min 150 words.\n"
        "MANDATORY: End with a 'Sources' block listing every Today's News item that supports "
        "your analysis, with exact x.com links. Format:\n"
        "Sources\n"
        "• [source description] → [x.com link]\n"
        "• ...\n]\n"
        "===INSIGHT_EN_END===\n\n"
        "===TWEET_START===\n"
