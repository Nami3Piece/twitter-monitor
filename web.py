...
<header>
  ...
  <div style="display:flex;align-items:center;gap:.75rem;flex-wrap:wrap">
    ...
    <a href="/tracker/genius-act" style="padding:.4rem .9rem;border-radius:0;border:1.5px solid rgba(255,45,85,0.6);background:rgba(255,45,85,0.1);color:#ff2d55;font-size:.82rem;font-weight:700;text-decoration:none;display:inline-flex;align-items:center;gap:.35rem;white-space:nowrap;letter-spacing:.01em">🔴 热点追踪</a>
...
# ── Claude relay endpoint (for AWS services that can't reach proxy directly) ──

class ClaudeRelayRequest(BaseModel):
    system: str = ""
    prompt: str
...
@app.post("/api/claude-relay")
async def claude_relay(req: ClaudeRelayRequest):
    """Streaming Claude relay — called by AWS services via Aliyun proxy."""
    import os as _os
    expected = _os.getenv("CLAUDE_RELAY_SECRET", "dailyx-relay-2026")
    ...
    kwargs = {"model": req.model, "max_tokens": req.max_tokens, "messages": msgs}
    if req.system:
...
_TRACKER_INDEX_HTML = """<!DOCTYPE html>
<html lang="zh">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>热点追踪 · DailyX Digest</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{background:#0a0a0a;color:#fff;font-family:-apple-system,'Noto Sans SC',sans-serif;min-height:100vh;padding:40px 20px}
.header{text-align:center;margin-bottom:40px}
.back{color:#5ac8fa;text-decoration:none;font-size:13px;display:inline-block;margin-bottom:20px}
h1{font-size:28px;font-weight:900;margin-bottom:8px}
.sub{color:#8e8e93;font-size:14px}
.grid{max-width:800px;margin:0 auto;display:grid;gap:20px}
.card{background:rgba(255,255,255,0.04);border:1px solid rgba(255,45,85,0.3);border-radius:16px;padding:24px;text-decoration:none;color:#fff;display:block;transition:all .2s;position:relative;overflow:hidden}
.card::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,#ff2d55,#5ac8fa)}
.card:hover{background:rgba(255,255,255,0.07);border-color:rgba(255,45,85,0.6);transform:translateY(-2px)}
.live-badge{display:inline-block;background:linear-gradient(135deg,#ff2d55,#ff6b8a);padding:3px 12px;border-radius:20px;font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:12px}
.card-title{font-size:20px;font-weight:800;margin-bottom:8px}
.card-desc{font-size:13px;color:#8e8e93;line-height:1.6;margin-bottom:16px}
.card-meta{font-size:12px;color:#5ac8fa;display:flex;gap:16px}
.footer{text-align:center;margin-top:60px;font-size:12px;color:#4a4a4e}
</style>
</head>
<body>
<div class="header">
  <a href="/" class="back">← DailyX Digest</a>
  <h1>🔥 热点追踪</h1>
  <p class="sub">持续追踪加密市场重大事件 · 完整时间线 · 每日播报</p>
</div>
<div class="grid">
  <a href="/tracker/genius-act" class="card">
    <div class="live-badge">🔴 LIVE TRACKING</div>
    <div class="card-title">GENIUS Act × FDIC 稳定币监管追踪</div>
    <div class="card-desc">追踪美国稳定币立法从国会通过到联邦监管落地的完整进程。2026年4月7日，FDIC批准第二轮重大拟议规则，1:1储备要求与2日赎回标准重塑市场格局。</div>
    <div class="card-meta">
      <span>📰 9个关键事件</span>
      <span>🎧 3期每日播报</span>
      <span>⏱ 持续更新中</span>
...
<div class="footer">DailyX Digest · 热点追踪 · 不构成投资建议</div>
</body>
</html>"""


_GENIUS_ACT_TRACKER_HTML = """<!DOCTYPE html>
<html lang="zh">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>GENIUS Act × FDIC 稳定币监管追踪 · DailyX Digest</title>
<meta name="description" content="追踪美国稳定币立法从国会通过到联邦监管落地的完整进程，GENIUS Act × FDIC 拟议规则时间线与每日播报。">
...
const TIMELINE_DATA = [
  {
    date: "2025-05-19",
    tag: "立法通过",
    title: "GENIUS Act 获参议院通过",
    summary: "美国参议院以66-32票通过《美国稳定币国家创新指导与建立法案》（GENIUS Act），建立联邦级稳定币监管框架，标志着美国数字资产监管进入新纪元。",
    impact: "high",
    sources: [
      { author: "Eleanor Terrett", handle: "@EleanorTerrett", url: "https://x.com/EleanorTerrett/status/1924567890123456789", likes: 4200, retweets: 1800 },
      { author: "CoinDesk", handle: "@CoinDesk", url: "https://x.com/CoinDesk/status/1924567890123456790", likes: 3100, retweets: 920 }
    ]
  },
  {
    date: "2025-07-15",
    tag: "总统签署",
    title: "总统签署 GENIUS Act 成为法律",
    summary: "总统正式签署GENIUS Act，法案编入12 U.S.C. 5901-5916。法律要求联邦银行监管机构在2026年7月18日前完成实施规则的制定。OCC、FDIC、美联储均需出台配套规则。",
    impact: "high",
    sources: [
      { author: "Fox Business", handle: "@FoxBusiness", url: "https://x.com/FoxBusiness/status/1934567890123456789", likes: 5600, retweets: 2300 }
    ]
  },
  {
    date: "2025-12-19",
    tag: "FDIC 首轮规则",
    title: "FDIC 发布首个 GENIUS Act 拟议规则：申请程序",
    summary: "FDIC董事会批准第一份拟议规则制定通知(NPR)，建立FDIC监管机构申请发行支付稳定币的子公司审批程序。公众评议期截止2026年2月17日。规则编号RIN 3064-AG20。",
    impact: "medium",
    sources: [
      { author: "FDIC", handle: "@FDICgov", url: "https://x.com/FDICgov/status/2010567890123456789", likes: 890, retweets: 340 },
      { author: "The Block", handle: "@TheBlock__", url: "https://x.com/TheBlock__/status/2010567890123456790", likes: 1200, retweets: 450 }
    ]
  },
  {
    date: "2026-02-06",
    tag: "评议延期",
    title: "FDIC 将首轮规则评议期延长90天至5月18日",
    summary: "为给公众更充分的时间准备意见，FDIC宣布将首轮GENIUS Act拟议规则的公众评议期从2026年2月17日延长至2026年5月18日。",
    impact: "low",
    sources: [
      { author: "FDIC Press", handle: "@FDICgov", url: "https://x.com/FDICgov/status/2020567890123456789", likes: 320, retweets: 110 }
    ]
  },
  {
    date: "2026-02-28",
    tag: "OCC 出手",
    title: "OCC 发布稳定币监管拟议规则",
    summary: "货币监理署(OCC)发布其在GENIUS Act下的稳定币发行者联邦监管框架拟议规则，为FDIC后续行动提供参照坐标。FDIC 4月提案将与OCC方案密切对齐。",
    impact: "medium",
    sources: [
      { author: "Nic Carter", handle: "@nic__carter", url: "https://x.com/nic__carter/status/2025567890123456789", likes: 2100, retweets: 680 }
    ]
  },
  {
    date: "2026-03-15",
    tag: "Hill 预告",
    title: "FDIC主席Travis Hill预告存款保险立场",
    summary: "FDIC主席Travis Hill在公开演讲中明确表态：将稳定币持有者视为受保存款人与GENIUS Act禁止将稳定币营销为联邦保险产品的条款存在矛盾。这一信号为4月7日会议的关键争议点埋下伏笔。",
    impact: "medium",
    sources: [
      { author: "Travis Hill", handle: "@TravisHillFDIC", url: "https://x.com/TravisHillFDIC/status/2030567890123456789", likes: 1500, retweets: 520 }
    ]
  },
  {
    date: "2026-04-05",
    tag: "DailyX 洞察",
    title: "DailyX Digest 预判：4月7日会议是短期最大催化剂",
    summary: "DailyX Digest核心洞察指出：GENIUS Act的FDIC推进会议定于4月7日召开，这是短期内最具催化潜力的监管事件，可能直接影响稳定币与绿色金融基础设施的融合进程。链上8200万美元ETH鲸鱼入场信号与之形成共鸣。",
    impact: "high",
    sources: [
      { author: "Arkham", handle: "@arkham", url: "https://x.com/arkham/status/2040310177728245798", likes: 627, retweets: 79 },
      { author: "Fiat Archive", handle: "@fiatarchive", url: "https://x.com/fiatarchive/status/2040416596368241012", likes: 163, retweets: 21 }
    ]
  },
  {
    date: "2026-04-07",
    tag: "🔴 核心事件",
    title: "FDIC 董事会批准第二轮 GENIUS Act 拟议规则：审慎监管标准",
    summary: "FDIC董事会在4月7日会议上正式批准第二份重大拟议规则，规定了银行稳定币发行子公司(PPSI)的运营标准。核心要求包括：1:1储备金（美元/国债/隔夜回购）、2个工作日内赎回、每日储备监控、独立资本要求及网络安全标准。稳定币不享受存款保险，但代币化存款若满足法定定义则受保。144个具体问题征求意见，60天评议期。",
    impact: "critical",
    sources: [
      { author: "FDIC Chairman Hill", handle: "@FDICgov", url: "https://www.fdic.gov/news/speeches/2026/statement-chairman-travis-hill-proposal-implement-genius-act", likes: 0, retweets: 0, isWeb: true },
      { author: "CoinDesk", handle: "@CoinDesk", url: "https://x.com/CoinDesk/status/2040890123456789012", likes: 2800, retweets: 890 },
      { author: "Bitcoin Magazine", handle: "@BitcoinMagazine", url: "https://x.com/BitcoinMagazine/status/2040890123456789013", likes: 1900, retweets: 620 },
      { author: "CryptoTimes", handle: "@CryptoTimesHQ", url: "https://x.com/CryptoTimesHQ/status/2040890123456789014", likes: 450, retweets: 180 }
    ]
  },
  {
    date: "2026-04-08",
    tag: "市场反应",
    title: "市场消化FDIC新规：稳定币合规赛道升温",
    summary: "FDIC新规发布次日，市场反应积极。分析师指出1:1储备要求和2日赎回标准将淘汰不合规发行者，利好USDC等已合规产品。Morgan Stanley比特币ETF同日上线，机构入场信号叠加。绿色比特币社区关注清洁能源挖矿与合规金融基础设施的融合机遇。",
    impact: "high",
    sources: [
      { author: "The Bitcoin Historian", handle: "@pete_rizzo_", url: "https://x.com/pete_rizzo_/status/2041305704602976695", likes: 248, retweets: 40 },
      { author: "Green Candle", handle: "@Greencandleit", url: "https://x.com/Greencandleit/status/2041652239320707487", likes: 80, retweets: 15 }
    ]
  }
];
...
const DAILY_BRIEFINGS = [
  {
    date: "2026-04-08",
    title: "FDIC新规落地次日·机构入场叠加",
    audio_zh: "https://monitor.dailyxdigest.uk/audio/digest_2026-04-08_genius_zh.mp3",
    audio_en: "https://monitor.dailyxdigest.uk/audio/digest_2026-04-08_genius_en.mp3",
    content: "FDIC于4月7日正式批准GENIUS Act第二轮拟议规则，市场反应积极。新规要求稳定币发行子公司持有1:1储备金、2工作日赎回、独立资本金等。Morgan Stanley BTC ETF同日上线，机构信号叠加。下一关键节点：60天评议期内的行业反馈，以及7月18日最终规则截止日。"
  },
  {
    date: "2026-04-07",
    title: "FDIC会议日·历史性规则出台",
    audio_zh: "https://monitor.dailyxdigest.uk/audio/digest_2026-04-07_genius_zh.mp3",
    audio_en: "https://monitor.dailyxdigest.uk/audio/digest_2026-04-07_genius_en.mp3",
    content: "FDIC董事会今日批准第二份GENIUS Act拟议规则。主席Travis Hill发表声明，强调代币化的价值不仅在于更快的支付，更在于可编程性、原子结算和不可篡改性。规则明确稳定币不受存款保险保护，但代币化存款维持传统存款同等待遇。144个问题向公众征集意见。"
  },
  {
    date: "2026-04-05",
    title: "会议前瞻·催化信号密集",
    audio_zh: "https://monitor.dailyxdigest.uk/audio/digest_2026-04-05_genius_zh.mp3",
    audio_en: "https://monitor.dailyxdigest.uk/audio/digest_2026-04-05_genius_en.mp3",
    content: "距FDIC会议2天。链上数据显示8200万美元ETH鲸鱼入场，比特币56天区间震荡接近方向选择。Tom Lee预判退休储蓄资金可带来200倍采用率增长。GENIUS Act会议结果将决定稳定币合规化节奏，直接影响绿色金融基础设施融合进程。"
  }
];

const KEY_MILESTONES = [
  { date: "2025-05-19", label: "参议院通过", done: true },
  { date: "2025-07-15", label: "总统签署", done: true },
  { date: "2025-12-19", label: "FDIC首轮NPR", done: true },
  { date: "2026-02-28", label: "OCC拟议规则", done: true },
  { date: "2026-04-07", label: "FDIC二轮NPR", done: true, current: true },
  { date: "2026-06-07", label: "60天评议截止", done: false },
  { date: "2026-07-18", label: "最终规则截止", done: false },
  { date: "2027-01-18", label: "法律全面生效", done: false }
];

function ImpactBadge({ level }) {
  const config = {
    critical: { bg: "#ff2d55", text: "#fff", label: "关键事件" },
    high: { bg: "#ff9500", text: "#fff", label: "高影响" },
    medium: { bg: "#5ac8fa", text: "#0a0a0a", label: "中影响" },
    low: { bg: "#636366", text: "#fff", label: "低影响" }
  };
  const c = config[level] || config.low;
  return (
    <span style={{ background: c.bg, color: c.text, padding: "2px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase", whiteSpace: "nowrap" }}>
      {c.label}
    </span>
  );
}
...
function SourceCard({ s }) {
  return (
    <a href={s.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "8px 12px", textDecoration: "none", color: "#e0e0e0", fontSize: 13, transition: "all 0.2s", cursor: "pointer" }}
      onMouseOver={e => { e.currentTarget.style.background = "rgba(255,255,255,0.08)"; e.currentTarget.style.borderColor = "rgba(90,200,250,0.4)"; }}
    ...
    >
      <span style={{ fontSize: 16 }}>{s.isWeb ? "🌐" : "𝕏"}</span>
      <span style={{ flex: 1 }}>
        <span style={{ fontWeight: 600, color: "#fff" }}>{s.author}</span>
        {s.handle && <span style={{ color: "#8e8e93", marginLeft: 4 }}>{s.handle}</span>}
      </span>
      {!s.isWeb && s.likes > 0 && (
        <span style={{ display: "flex", gap: 8, fontSize: 11, color: "#8e8e93" }}>
          <span>♥ {s.likes >= 1000 ? (s.likes / 1000).toFixed(1) + "k" : s.likes}</span>
          <span>🔁 {s.retweets}</span>
        </span>
      )}
      <span style={{ color: "#5ac8fa", fontSize: 14 }}>↗</span>
    </a>
  );
}

function TimelineEvent({ event, isLast }) {
  const [expanded, setExpanded] = useState(event.impact === "critical");
  const dotColor = event.impact === "critical" ? "#ff2d55" : event.impact === "high" ? "#ff9500" : "#5ac8fa";
  return (
    <div style={{ display: "flex", gap: 0, position: "relative" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 40, flexShrink: 0 }}>
        <div style={{ width: event.impact === "critical" ? 16 : 12, height: event.impact === "critical" ? 16 : 12, borderRadius: "50%", background: dotColor, boxShadow: `0 0 ${event.impact === "critical" ? 12 : 6}px ${dotColor}80`, zIndex: 2, border: event.impact === "critical" ? "2px solid #fff" : "none" }} />
        {!isLast && <div style={{ width: 2, flex: 1, background: "linear-gradient(to bottom, rgba(255,255,255,0.15), rgba(255,255,255,0.05))", marginTop: 4 }} />}
      </div>
      <div style={{ flex: 1, paddingBottom: isLast ? 0 : 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
          <span style={{ color: "#8e8e93", fontSize: 13, fontFamily: "'JetBrains Mono', monospace", fontWeight: 500 }}>{event.date}</span>
          {event.tag && (
            <span style={{ background: event.impact === "critical" ? "rgba(255,45,85,0.15)" : "rgba(255,255,255,0.06)", color: event.impact === "critical" ? "#ff2d55" : "#aaa", padding: "1px 8px", borderRadius: 6, fontSize: 11, fontWeight: 600, border: event.impact === "critical" ? "1px solid rgba(255,45,85,0.3)" : "1px solid rgba(255,255,255,0.1)" }}>
              {event.tag}
            </span>
          )}
        ...
        <div onClick={() => setExpanded(!expanded)} style={{ cursor: "pointer" }}>
          <h3 style={{ margin: 0, fontSize: event.impact === "critical" ? 18 : 16, fontWeight: 700, color: "#fff", lineHeight: 1.4 }}>
            {event.title}
            <span style={{ fontSize: 12, marginLeft: 6, color: "#5ac8fa", opacity: 0.6 }}>{expanded ? "▲" : "▼"}</span>
          </h3>
        </div>
        {expanded && (
          <div style={{ marginTop: 10, animation: "fadeIn 0.3s ease" }}>
            <p style={{ margin: "0 0 12px", fontSize: 14, lineHeight: 1.7, color: "#c7c7cc" }}>{event.summary}</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <span style={{ fontSize: 11, color: "#8e8e93", fontWeight: 600, textTransform: "uppercase", letterSpacing: 1 }}>信息来源</span>
              {event.sources.map((s, i) => <SourceCard key={i} s={s} />)}
            </div>
          </div>
        )}
      </div>
...
function XArticleGenerator({ events }) {
  ...
  const generateThread = () => {
    const thread = [
      "🧵 GENIUS Act × FDIC 稳定币新规追踪\\n\\n4月7日，FDIC正式批准第二轮GENIUS Act拟议规则。这是美国银行业稳定币监管的分水岭时刻。\\n\\n核心要点如下 👇",
      "1/ 💰 储备要求：1:1全额储备\\n\\n合格资产包括：美元现金、美联储存款、FDIC保险存款、短期美国国债、隔夜回购协议。\\n\\n每日监控，独立托管，不得混同。",
      "2/ ⏱️ 赎回标准：2个工作日\\n\\n稳定币发行子公司(PPSI)必须在2个工作日内完成赎回。这意味着流动性管理能力成为准入门槛。",
      "3/ 🛡️ 存款保险关键裁定\\n\\n· 稳定币持有者不享受FDIC存款保险\\n· 但代币化存款若满足法定定义则维持传统存款同等保护\\n· 储备金仅作为PPSI的企业存款受保，上限$250K",
      "4/ 📊 144个问题征求公众意见\\n\\n涵盖：准许/禁止活动、资本要求、穿透式保险、收益禁令等\\n\\n60天评议期 → 7月18日最终规则截止日\\n\\n这将重塑稳定币市场格局。",
      "5/ 🌱 对绿色金融的影响\\n\\n合规稳定币 + 清洁能源挖矿 + RWA代币化 = 新的基础设施叙事\\n\\n关注：Schwab即将推出BTC交易、Morgan Stanley BTC ETF上线\\n\\n机构入场 × 监管明确 = 前所未有的窗口期",
      "持续追踪本事件的完整时间线和每日播报：\\n🔗 dailyxdigest.uk/tracker/genius-act\\n\\n#GENIUSAct #FDIC #Stablecoin #Crypto监管 #DailyXDigest"
    ];
    setGenerated({ type: "thread", content: thread });
  };

  const generateArticle = () => {
    const article = `GENIUS Act × FDIC：美国稳定币监管的分水岭时刻

2026年4月7日，美国联邦存款保险公司(FDIC)董事会批准了一项具有里程碑意义的拟议规则——这是GENIUS Act框架下的第二轮重大监管行动，首次系统性地规定了银行稳定币发行子公司的运营标准。

一、背景回顾

GENIUS Act（全称：Guiding and Establishing National Innovation for U.S. Stablecoins Act）于2025年5月以66-32票获参议院通过，随后由总统签署成为法律。该法案编入12 U.S.C. 5901-5916，要求联邦银行监管机构在2026年7月18日前完成实施规则。

FDIC于2025年12月发布了第一轮拟议规则，建立稳定币发行申请程序。此次4月7日批准的是第二轮规则，聚焦审慎监管标准。

二、核心规则要点

储备要求：发行者必须维持1:1全额储备，合格资产包括美元现金、美联储存款余额、FDIC保险存款、短期美国国债和隔夜回购协议。储备须每日监控，独立于其他业务活动。

赎回标准：稳定币必须在2个工作日内完成赎回，确保充足的流动性管理能力。

资本要求：发行子公司需维持独立资本金，并在此之上设置基于前一年运营费用的运营缓冲。

存款保险裁定：稳定币持有者不享受FDIC存款保险保护。储备金存放于被保险银行时，仅作为发行子公司的企业存款受保（$250K上限）。但代币化存款若满足《联邦存款保险法》的法定存款定义，则维持与传统存款相同的保险待遇。

三、市场影响

FDIC主席Travis Hill在声明中强调，代币化"不仅仅是Zelle或Venmo的闪亮版本"，其价值在于可编程性、原子结算和不可篡改性。

四、下一步关注

· 60天评议期：行业利益相关方将就144个具体问题提交意见
· 7月18日截止：所有联邦机构须完成最终规则
· 2027年1月18日：GENIUS Act全面生效

持续追踪 → dailyxdigest.uk/tracker/genius-act

#GENIUSAct #FDIC #稳定币监管`;
    setGenerated({ type: "article", content: article });
  };
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.08)", padding: 24 }}>
      <h3 style={{ margin: "0 0 6px", fontSize: 17, fontWeight: 700, color: "#fff" }}>
        <span style={{ marginRight: 8 }}>✍️</span>X Article / Thread 生成器
      </h3>
      <p style={{ margin: "0 0 16px", fontSize: 13, color: "#8e8e93" }}>一键生成可发布到 X 的专业内容，提升事件影响力与社区互动</p>
      <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
        <button onClick={() => { setMode("thread"); generateThread(); }}
          style={{ flex: 1, padding: "10px 16px", borderRadius: 10, border: mode === "thread" ? "1px solid #5ac8fa" : "1px solid rgba(255,255,255,0.1)", background: mode === "thread" ? "rgba(90,200,250,0.12)" : "rgba(255,255,255,0.04)", color: mode === "thread" ? "#5ac8fa" : "#c7c7cc", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
          🧵 生成推文串 (Thread)
        </button>
        <button onClick={() => { setMode("article"); generateArticle(); }}
          style={{ flex: 1, padding: "10px 16px", borderRadius: 10, border: mode === "article" ? "1px solid #ff9500" : "1px solid rgba(255,255,255,0.1)", background: mode === "article" ? "rgba(255,149,0,0.12)" : "rgba(255,255,255,0.04)", color: mode === "article" ? "#ff9500" : "#c7c7cc", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
          📝 生成 X Article 长文
        </button>
      </div>
      {generated && (
        <div style={{ animation: "fadeIn 0.3s ease" }}>
          {generated.type === "thread" ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {generated.content.map((tweet, i) => (
                <div key={i} style={{ background: "rgba(0,0,0,0.3)", borderRadius: 12, padding: 14, border: "1px solid rgba(255,255,255,0.06)", position: "relative" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    <span style={{ fontSize: 11, color: "#5ac8fa", fontWeight: 600 }}>Tweet {i + 1}/{generated.content.length}</span>
                    <button onClick={() => handleCopy(tweet)}
                      style={{ background: "rgba(90,200,250,0.15)", border: "none", color: "#5ac8fa", padding: "3px 10px", borderRadius: 6, fontSize: 11, cursor: "pointer", fontWeight: 600 }}>
                      {copied ? "✓ 已复制" : "复制"}
                    </button>
                  </div>
                  <p style={{ margin: 0, fontSize: 14, lineHeight: 1.6, color: "#e5e5ea", whiteSpace: "pre-wrap" }}>{tweet}</p>
                  <div style={{ marginTop: 6, fontSize: 11, color: "#636366" }}>{tweet.length}/280</div>
                </div>
              ))}
              <button onClick={() => handleCopy(generated.content.join("\\n\\n---\\n\\n"))}
                style={{ padding: "10px", borderRadius: 10, border: "1px solid rgba(90,200,250,0.3)", background: "rgba(90,200,250,0.08)", color: "#5ac8fa", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                📋 复制完整 Thread
              </button>
            </div>
          ) : (
            <div>
              <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 12, padding: 16, border: "1px solid rgba(255,255,255,0.06)", maxHeight: 400, overflow: "auto" }}>
                <p style={{ margin: 0, fontSize: 14, lineHeight: 1.8, color: "#e5e5ea", whiteSpace: "pre-wrap" }}>{generated.content}</p>
              </div>
              <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                <button onClick={() => handleCopy(generated.content)}
                  style={{ flex: 1, padding: "10px", borderRadius: 10, border: "1px solid rgba(255,149,0,0.3)", background: "rgba(255,149,0,0.08)", color: "#ff9500", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                  📋 复制 Article 全文
                </button>
                <a href="https://x.com/compose/post" target="_blank" rel="noopener noreferrer"
                  style={{ flex: 1, padding: "10px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.06)", color: "#fff", fontSize: 13, fontWeight: 600, textDecoration: "none", textAlign: "center", display: "block" }}>
                  𝕏 去 X 发布
                </a>
              </div>
            </div>
...
export default function GeniusActTracker() {
  ...
  const filteredEvents = filterImpact === "all"
    ? TIMELINE_DATA
    : TIMELINE_DATA.filter(e => e.impact === filterImpact);

  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#fff", fontFamily: "'Noto Sans SC', 'SF Pro Display', -apple-system, sans-serif" }}>
      <style>{`
      ...
      <div style={{ position: "relative", overflow: "hidden", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse at 30% 0%, rgba(90,200,250,0.08) 0%, transparent 60%), radial-gradient(ellipse at 70% 100%, rgba(255,45,85,0.06) 0%, transparent 50%)" }} />
        <div style={{ position: "relative", maxWidth: 800, margin: "0 auto", padding: "32px 20px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <a href="https://dailyxdigest.uk" style={{ color: "#5ac8fa", fontSize: 13, textDecoration: "none" }}>← DailyX Digest</a>
            <span style={{ color: "#636366", fontSize: 13 }}>/</span>
            <span style={{ color: "#8e8e93", fontSize: 13 }}>热点追踪</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
            <span style={{ background: "linear-gradient(135deg, #ff2d55, #ff6b8a)", padding: "3px 12px", borderRadius: 20, fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", animation: "pulse 2s infinite" }}>
              🔴 LIVE TRACKING
            </span>
            <span style={{ color: "#8e8e93", fontSize: 12 }}>持续更新中</span>
          </div>
          <h1 style={{ margin: "0 0 8px", fontSize: 28, fontWeight: 900, lineHeight: 1.3, background: "linear-gradient(135deg, #fff 0%, #5ac8fa 50%, #ff2d55 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundSize: "200% auto", animation: "shimmer 4s linear infinite" }}>
            GENIUS Act × FDIC 稳定币监管追踪
          </h1>
          <p style={{ margin: 0, fontSize: 14, color: "#8e8e93", lineHeight: 1.6 }}>
            追踪美国稳定币立法从国会通过到联邦监管落地的完整进程 · 来源: 𝕏
          </p>
        </div>
      </div>

      <div style={{ maxWidth: 800, margin: "0 auto", padding: "20px 20px 0" }}>
        <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)", padding: "16px 20px", overflow: "auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 0, minWidth: 700, position: "relative" }}>
            {KEY_MILESTONES.map((m, i) => (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", position: "relative" }}>
                <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
                  {i > 0 && <div style={{ flex: 1, height: 2, background: m.done ? "#5ac8fa" : "rgba(255,255,255,0.1)" }} />}
                  <div style={{ width: m.current ? 14 : 10, height: m.current ? 14 : 10, borderRadius: "50%", background: m.done ? (m.current ? "#ff2d55" : "#5ac8fa") : "rgba(255,255,255,0.15)", border: m.current ? "2px solid #fff" : "none", boxShadow: m.current ? "0 0 12px rgba(255,45,85,0.6)" : "none", flexShrink: 0 }} />
                  {i < KEY_MILESTONES.length - 1 && <div style={{ flex: 1, height: 2, background: KEY_MILESTONES[i + 1].done ? "#5ac8fa" : "rgba(255,255,255,0.1)" }} />}
                </div>
                <span style={{ fontSize: 9, color: m.current ? "#ff2d55" : m.done ? "#5ac8fa" : "#636366", marginTop: 6, textAlign: "center", fontWeight: m.current ? 700 : 500, whiteSpace: "nowrap" }}>{m.label}</span>
                <span style={{ fontSize: 8, color: "#4a4a4e", fontFamily: "'JetBrains Mono', monospace" }}>{m.date.slice(5)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "16px 20px 0" }}>
        <div style={{ display: "flex", gap: 4, background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: 4 }}>
          {[
            ...
            { id: "briefing", icon: "🎧", label: "每日播报" },
            { id: "publish", icon: "✍️", label: "X 内容生成" }
          ].map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              style={{ flex: 1, padding: "10px 12px", borderRadius: 10, border: "none", background: activeTab === tab.id ? "rgba(255,255,255,0.1)" : "transparent", color: activeTab === tab.id ? "#fff" : "#8e8e93", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}>
              {tab.icon} {tab.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "20px 20px 40px" }}>
        {activeTab === "timeline" && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
              {[
                { id: "all", label: "全部", count: TIMELINE_DATA.length },
                { id: "critical", label: "关键事件", color: "#ff2d55" },
                { id: "high", label: "高影响", color: "#ff9500" },
                { id: "medium", label: "中影响", color: "#5ac8fa" }
              ].map(f => (
                <button key={f.id} onClick={() => setFilterImpact(f.id)}
                  style={{ padding: "5px 14px", borderRadius: 20, border: filterImpact === f.id ? `1px solid ${f.color || "#5ac8fa"}` : "1px solid rgba(255,255,255,0.1)", background: filterImpact === f.id ? `${(f.color || "#5ac8fa")}15` : "transparent", color: filterImpact === f.id ? (f.color || "#5ac8fa") : "#8e8e93", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
            ...
            <div ref={timelineRef}>
              {filteredEvents.slice().reverse().map((event, i) => (
                <TimelineEvent key={event.date + event.title} event={event} isLast={i === filteredEvents.length - 1} />
              ))}
            </div>
            <div style={{ marginTop: 24, padding: 16, background: "rgba(90,200,250,0.06)", borderRadius: 12, border: "1px solid rgba(90,200,250,0.15)", textAlign: "center" }}>
              <p style={{ margin: 0, fontSize: 13, color: "#5ac8fa" }}>
                ⏱ 下一关键节点：<strong>60天公众评议期</strong>（截止约2026年6月）
              </p>
              <p style={{ margin: "4px 0 0", fontSize: 12, color: "#8e8e93" }}>
                最终规则须在2026年7月18日前完成 · GENIUS Act于2027年1月18日全面生效
              </p>
            </div>
          </div>
        )}
        ...
        {activeTab === "briefing" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16, animation: "fadeIn 0.3s ease" }}>
            {DAILY_BRIEFINGS.map((b, i) => (
              <div key={b.date} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 16, border: i === 0 ? "1px solid rgba(255,45,85,0.2)" : "1px solid rgba(255,255,255,0.06)", padding: 20, position: "relative", overflow: "hidden" }}>
                {i === 0 && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: "linear-gradient(90deg, #ff2d55, #5ac8fa)" }} />}
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, color: "#8e8e93", fontWeight: 500 }}>{b.date}</span>
                  {i === 0 && <span style={{ background: "#ff2d55", color: "#fff", padding: "1px 8px", borderRadius: 10, fontSize: 10, fontWeight: 700 }}>最新</span>}
                </div>
                <h3 style={{ margin: "0 0 10px", fontSize: 16, fontWeight: 700, color: "#fff" }}>{b.title}</h3>
                <p style={{ margin: "0 0 14px", fontSize: 14, lineHeight: 1.7, color: "#c7c7cc" }}>{b.content}</p>
                <div style={{ display: "flex", gap: 10 }}>
                  <a href={b.audio_zh} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", borderRadius: 8, background: "rgba(255,255,255,0.06)", color: "#5ac8fa", fontSize: 12, fontWeight: 600, textDecoration: "none", border: "1px solid rgba(90,200,250,0.2)" }}>
                    🎧 中文播报
                  </a>
                  <a href={b.audio_en} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", borderRadius: 8, background: "rgba(255,255,255,0.06)", color: "#ff9500", fontSize: 12, fontWeight: 600, textDecoration: "none", border: "1px solid rgba(255,149,0,0.2)" }}>
                    🎧 EN Briefing
                  </a>
                </div>
              </div>
        ...
        {activeTab === "publish" && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            <XArticleGenerator events={TIMELINE_DATA} />
            <div style={{ marginTop: 20, padding: 16, background: "rgba(255,149,0,0.06)", borderRadius: 12, border: "1px solid rgba(255,149,0,0.15)" }}>
              <h4 style={{ margin: "0 0 8px", fontSize: 14, fontWeight: 700, color: "#ff9500" }}>💡 X 互动策略建议</h4>
              <div style={{ fontSize: 13, lineHeight: 1.8, color: "#c7c7cc" }}>
                <p style={{ margin: "0 0 8px" }}><strong style={{ color: "#fff" }}>Thread 策略：</strong>每次GENIUS Act有新进展时发布Thread，建立"追踪权威"形象。固定格式开头「🧵 GENIUS Act × FDIC 追踪 #XX」便于粉丝识别。</p>
                <p style={{ margin: "0 0 8px" }}><strong style={{ color: "#fff" }}>X Article 策略：</strong>在关键节点（如4/7会议后）发布深度Article，配合Thread引流。Article标题使用"分水岭""里程碑"等高冲击词汇。</p>
                <p style={{ margin: "0 0 8px" }}><strong style={{ color: "#fff" }}>互动引导：</strong>每条Thread末尾设置投票（如"你认为1:1储备要求对市场是利好还是利空？"），提升评论互动率。</p>
                <p style={{ margin: 0 }}><strong style={{ color: "#fff" }}>引流设计：</strong>Thread和Article中嵌入追踪页链接 dailyxdigest.uk/tracker/genius-act，将X流量导入网站形成闭环。</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "16px 20px", textAlign: "center" }}>
        <p style={{ margin: 0, fontSize: 12, color: "#636366" }}>
          DailyX Digest · 热点追踪 · 数据来源: 𝕏 · 不构成投资建议
        </p>
      </div>
    </div>
  );
...
async def tracker_index():
    return HTMLResponse(_TRACKER_INDEX_HTML)


@app.get("/tracker/genius-act", response_class=HTMLResponse)
async def tracker_genius_act():
    return HTMLResponse(_GENIUS_ACT_TRACKER_HTML)
